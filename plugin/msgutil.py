...
class MsgUtil:
    ...
    def sendCard(self, Msg, suid, ruid):
        ...
        post_data = {'msg[sender_uid]': suid, 'msg[receiver_id]': ruid, 'msg[receiver_type]': 1, 'msg[msg_type]': 12,
                     'msg[content]': str(postJson), 'msg[timestamp]': round(thisTime), 'msg[new_face_version]': 0,
    ...
    def sendMsg(self, Msg, suid, ruid):
        ...
        post_data = {'msg[sender_uid]': suid, 'msg[receiver_id]': ruid, 'msg[receiver_type]': 1, 'msg[msg_type]': 1,
                     'msg[content]': str(postJson), 'msg[timestamp]': round(thisTime), 'msg[new_face_version]': 0,
                     'from_firework': 0, 'build': 0, 'mobi_app': 'web', 'csrf_token': self.csrf_token,
                     'csrf': self.csrf_token, 'msg[dev_id]': '10428CA1-33BC-4D9F-9247-62A54251D311',
                     'msg[msg_status]': 0}
    ...
    def sendImage(self, imageUrl, suid, ruid, width, height, format):
        thisTime = time.time()
        postJson = '{"url":"' + str(imageUrl) + '","imageType":"' + str(format) + '","original":1,"height":' + str(
            height) + ',"width":' + str(width) + '}'
        post_data = {'msg[sender_uid]': suid, 'msg[receiver_id]': ruid, 'msg[receiver_type]': 1, 'msg[msg_type]': 2,
                     'msg[content]': str(postJson), 'msg[timestamp]': round(thisTime), 'msg[new_face_version]': 0,
                     'from_firework': 0, 'build': 0, 'mobi_app': 'web', 'csrf_token': self.csrf_token,
                     'csrf': self.csrf_token, 'msg[dev_id]': '10428CA1-33BC-4D9F-9247-62A54251D311',
                     'msg[msg_status]': 0}
    ...
    def updateAck(self, Mid, csrf_token):
        ...
        fa = requests.post('https://api.vc.bilibili.com/session_svr/v1/session_svr/update_ack', post_data,
    ...
    def captureIamgeUrl(self, url):
        ...
        fa = requests.post('https://api.bilibili.com/x/article/creative/article/capture', post_data,
                           headers=self.headers)
        fa.encoding = 'utf-8'
        print(fa.text)
    ...
    def liveSendMsg(self, Msg, rommid):
        Msg = Msg.replace("﻿", "")
        thisTime = time.time()
        post_data = {'msg': Msg, 'roomid': rommid, 'bubble': 0, 'color': 16777215, 'fontsize': 25, 'mode': 1,
                     'rnd': round(thisTime), 'csrf': self.csrf_token, 'csrf_token': self.csrf_token}
